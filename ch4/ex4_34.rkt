...
;;; Ex 4.33 Make quote work with lazy lists.
;;;
;;; Works by expanding and evaluating quoted lists with whatever is
;;; installed as the current definition of cons (in this problem, lazy
;;; cons). See example at the end of this file.
(define (text-of-quotation exp env)
  (if (pair? (cadr exp))
      (eval (list 'cons (list 'quote (caadr exp)) (list 'quote (cdadr exp))) env)
      (cadr exp)))
...
(define primitive-procedures
  (list ;(list 'car car)
        ...
        (list '= =)
	;; lazy lists
	(list 'cons (lambda (x y) (cons 'cons (lambda (m) (m x y)))))
	(list 'car (lambda (z) ((cdr z) (lambda (p q) p))))
	(list 'cdr (lambda (z) ((cdr z) (lambda (p q) q))))
        ))
...
(define (user-print object)
  (cond ((lazy-cons? object) (display-lazy-cons object))
...
(define (lazy-cons? object) (tagged-list? object 'cons))
