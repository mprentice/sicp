...
(define (cond-actions clause)
  (if (null? (cdr clause))
      (cdr clause)
      ;; Special case to handle (<test> => <recipient>) cond syntax.
      ;; Not safe with side effects! Will cause side effects to occur
      ;; twice.
      (if (eq? (cadr clause) '=>)
          (if (null? (cddr clause))
              (error "=> must be followed by a function name -- COND->IF"
                     clause)
