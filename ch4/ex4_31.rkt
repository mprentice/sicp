...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)
         (maybe-lazy-apply (actual-value (operator exp) env)
			   (operands exp)
			   env))
...
;;; all the arguments.
(define (maybe-lazy-apply procedure arguments env)
  (cond ((primitive-procedure? procedure)
        ...
        (else (error
	       ("Unknown procedure type -- MAYBE-LAZY-APPLY" procedure)))))

;;; (a (b lazy) c (d lazy-memo)) -> (a b c d)
(define (procedure-parameter-symbols params)
  (if (no-parameters? params)
      '()
      (cons (if (pair? (first-parameter params))
		(car (first-parameter params))
		(first-parameter params))
	    (procedure-parameter-symbols (rest-parameters params)))))
...
(define (list-of-maybe-delayed-args params exps env)
  (if (no-operands? exps)
      '()
      (let ((exp (first-operand exps))
	    (param (first-parameter params)))
	(cons (cond ((lazy-memo-argument? param) (delay-it exp env true))
		    ((lazy-argument? param) (delay-it exp env false))
		    (else (eval exp env)))
	      (list-of-maybe-delayed-args (rest-parameters params)
					  (rest-operands exps)
					  env)))))
...
(define (lazy-memo-argument? param)
  (and (pair? param) (eq? (cadr param) 'lazy-memo)))

(define (lazy-argument? param)
...
;; ;;; L-Eval value:
;; ok
;; > (f (begin (display '(a strict)) (newline) 'a)
;;      (begin (display '(b lazy)) (newline) 'b)
;;      (begin (display '(c strict)) (newline) 'c)
;;      (begin (display '(d lazy-memo)) (newline) 'd))
;; (a strict)
;; (c strict)
;; (b lazy)
;; (d lazy-memo)
