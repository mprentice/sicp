...
(eval
 '(define (eva-map f l)
    (if (null? l)
        (quote ())
        (cons (f (car l)) (eva-map f (cdr l)))))
 the-global-environment)

;;; Ex 4.14 discussion
;;;
;;; `eva-map' is implemented in our evaluator, and runs fine. However,
;;; `louis-map' is implemented as a primitive calling the underlying
;;; system `map'.
;;;
;;; Call the system evaluator `s-eval' and our evaluator `m-eval'.
;;; `louis-map' doesn't work because it tries to call `map' in
;;; `s-eval' with a function in `m-eval', but `s-eval' expects a
;;; function in `s-eval'.
;;;
;;; `eva-map' is implemented in `m-eval' and uses functions in
;;; `m-eval', so there's no problem.
