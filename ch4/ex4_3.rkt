...
(define dispatch-table
  (list
   ...
   (cons variable? lookup-variable-value)
...
(define (eval exp env)
  (define (eval-this exp env dispatch-table)
    (if (null? dispatch-table)
        (error "Unknown expression type -- EVAL" exp)
        (let ((test-function (caar dispatch-table))
              (apply-function (cdar dispatch-table)))
          (if (test-function exp)
              (apply-function exp env)
              (eval-this exp env (cdr dispatch-table))))))
  (eval-this exp env dispatch-table))
