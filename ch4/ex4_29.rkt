...
(eval '(fib 26) the-global-environment)
...
;;; Discussion:
;;;
;;; The memoized version of (fib 26) performs noticeably faster than
;;; the non-memoized version. (on a 2014 iMac). This is because the
;;; non-memoized version has to re-compute n 4 times at each level of
;;; the recursion instead of just once. Carry that out for each level
;;; of recursion and it adds up to a significant performance overhead.
;;;
;;; Memoized square count = 1
;;; Unmemoized square count = 2
