...
(define (frame-scan var vars vals null-fn eq-fn)
  (cond ((null? vars) (null-fn))
        ((eq? var (car vars)) (eq-fn vars vals))
        (else (frame-scan var (cdr vars) (cdr vals) null-fn eq-fn))))

(define (env-loop var env eq-fn error-message)
  (if (eq? env the-empty-environment)
      (error error-message var)
      (let ((frame (first-frame env)))
        (define (null-fn)
          (env-loop var (enclosing-environment env) eq-fn error-message))
        (frame-scan var
...
(define (define-variable! var val env)
  (let ((frame (first-frame env)))
    (frame-scan var
                (frame-variables frame)
                (frame-values frame)
                (lambda () (add-bindings-to-frame! var val frame))
                (lambda (vars vals) (set-car! vals val)))))
