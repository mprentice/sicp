...
;;; Ex 4.16 installing `scan-out-defines' in `make-procedure' so it
...
(define (filter p l)
  (define (iter l acc)
    (cond ((null? l) acc)
          ((p (car l)) (iter (cdr l) (cons (car l) acc)))
          (else (iter (cdr l) acc))))
  (reverse (iter l '())))

(define (negate p) (lambda (e) (not (p e))))

(define (scan-out-defines body)
  (let ((defs (filter definition? body))
        (rest (filter (negate definition?) body)))
    (make-let
     ;; bindings
     (map (lambda (exp)
...
;;;     <rest of body of f>))
;;;
;;; environment for <rest of body of f>:
;;;
;;; ============                               ========
;;; = letrec   = ----------------------------> = f    =
;;; ============                               ========
...
;;;    |     +-------------+
;;; ===========         ==========
;;; = even? lambda =         = odd? lambda =
;;; ===========         ==========
;;; = n       =         = n      =
;;; ===========         ==========
;;;
;;; As we can see, the parent frame for the lambda frames is the letrec
;;; frame that has bindings for even? and odd?, so the even? and odd?
;;; references in the lambda bodies resolve correctly and there is no
;;; problem with mutual recursion. However, for let:
;;;
;;; ============                               ========
;;; = let      = ----------------------------> = f    =
;;; ============       ===========             ========
;;; = even? -- = ----> = even? lambda = ----------> = x: 5 =
;;; = odd?  -- = --+   ===========             ========
;;; ============   |   = n       =              ^
;;;                |   ===========              |
;;;                |                            |
...
;;;                    ==========
;;;
;;; There is no mutual recursion possible here since in the lambda frames
;;; even? and odd? don't have bindings.
